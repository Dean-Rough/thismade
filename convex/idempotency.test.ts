...
describe("idempotencyKeys.beginOrReplay", () => {
  it("claims a fresh key, then replays the completed response on a duplicate request", async () => {
    ...
    const first = await t.mutation(api.idempotencyKeys.beginOrReplay, {
      businessId,
    ...
    await t.mutation(api.idempotencyKeys.complete, {
      id: first.id,
      responseStatus: 200,
    ...
    const duplicate = await t.mutation(api.idempotencyKeys.beginOrReplay, {
      businessId,
      route: "PATCH /v1/business",
      key: "req-123",
      requestHash: "hash-of-body",
  ...
  it("rejects the same key reused with a different request body", async () => {
    ...
    const reused = await t.mutation(api.idempotencyKeys.beginOrReplay, {
      businessId,
      route: "PATCH /v1/business",
      key: "req-456",
  ...
  it("rejects a concurrent request that reuses a key still in progress", async () => {
    ...
    const concurrent = await t.mutation(api.idempotencyKeys.beginOrReplay, {
      businessId,
      route: "PATCH /v1/business",
      key: "req-789",
      requestHash: "hash-of-body",
  ...
  it("scopes keys per business + route, so the same key string is independent across businesses", async () => {
    ...
    const forA = await t.mutation(api.idempotencyKeys.beginOrReplay, {
      businessId: businessAId,
      route: "PATCH /v1/business",
      key: "shared-key",
      requestHash: "hash",
    });
    const forB = await t.mutation(api.idempotencyKeys.beginOrReplay, {
