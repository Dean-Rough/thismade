...
describe("tenancy: cross-business access", () => {
  it("returns null (never the document) when a business fetches another business's api key by id", async () => {
    ...
    const apiKeyAId = await t.mutation(internal.apiKeys.create, {
    ...
    const ownFetch = await t.query(internal.apiKeys.getScopedById, {
      apiKeyId: apiKeyAId,
      businessId: businessAId,
    });
    ...
    const crossTenantFetch = await t.query(internal.apiKeys.getScopedById, {
      apiKeyId: apiKeyAId,
  ...
  it("never returns another business's row from a genuinely nonexistent id either, so the two cases are indistinguishable", async () => {
    ...
    const apiKeyBId = await t.mutation(internal.apiKeys.create, {
      businessId: businessBId,
      name: "B's key",
      prefix: "tm_test_bbbb",
    ...
    const result = await t.query(internal.apiKeys.getScopedById, {
      apiKeyId: apiKeyBId,
      businessId: businessAId,
    });
  ...
  it("businesses:getSelf only ever returns the business matching the given id — no cross-tenant leak path", async () => {
    ...
    const selfA = await t.query(internal.businesses.getSelf, { businessId: businessAId });
    const selfB = await t.query(internal.businesses.getSelf, { businessId: businessBId });
