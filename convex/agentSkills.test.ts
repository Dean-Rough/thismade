...
describe("agentSkills: upsert bumps version per (business, skillKey)", () => {
  it("replaces content and increments version on a second upsert", async () => {
    ...
    const first = await t.mutation(internal.agentSkills.upsert, {
      businessId,
      skillKey: "brandkit",
      content: "draft v1",
    });
    ...
    const second = await t.mutation(internal.agentSkills.upsert, {
      businessId,
      skillKey: "brandkit",
      content: "draft v2",
    });
    ...
    const skill = await t.query(internal.agentSkills.get, { businessId, skillKey: "brandkit" });
    expect(skill?.content).toBe("draft v2");
    expect(skill?.version).toBe(2);

    const all = await t.query(internal.agentSkills.listByBusiness, { businessId });
  ...
  it("attaches new skills without any predefined key set", async () => {
    ...
    await t.mutation(internal.agentSkills.upsert, {
      businessId,
...
describe("agentSkills: tenancy", () => {
  it("never returns another business's skill for the same skillKey", async () => {
    ...
    const bSkill = await t.query(internal.agentSkills.get, {
